...
export default function LearningSection2({ title, description, image, animation }) {
  return (
    <section className="flex flex-col md:flex-row items-center justify-between min-h-screen px-24 py-16">
      {/* Text Side */}
      <motion.div
        className="md:w-1/2"
      ...
      <motion.div
        ...
        viewport={{ once: true }}
      >
        <img src={image} alt={title} className="rounded-full w-[400px] h-[400px] object-cover" />
      </motion.div>
    </section>
  );
}
